import type { ChatRequest, FunctionCall, JSONValue, Message, ToolCall } from './types';
...
export async function processChatStream({
	...
	getStreamedResponse: () => Promise<Message | { messages: Message[]; data: JSONValue[] }>;
	experimental_onFunctionCall?: (
	...
	while (true) {
		...
		if ('messages' in messagesAndDataOrJustMessage) {
			...
			for (const message of messagesAndDataOrJustMessage.messages) {
				...
				if (
					(message.function_call === undefined || typeof message.function_call === 'string') &&
					(message.tool_calls === undefined || typeof message.tool_calls === 'string')
				) {
					continue;
				}
				...
				if (experimental_onFunctionCall) {
					...
					const functionCallResponse: ChatRequest | void = await experimental_onFunctionCall(
						getCurrentMessages(),
						functionCall
					);
				...
				if (experimental_onToolCall) {
					...
					// The "arguments" key of the function call object will still be a string which will have to be parsed in the function handler.
					// If the "arguments" JSON is malformed due to model error the user will have to handle that themselves.
					const toolCallResponse: ChatRequest | void = await experimental_onToolCall(
						getCurrentMessages(),
						toolCalls
					);
